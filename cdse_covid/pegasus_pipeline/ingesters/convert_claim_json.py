...
def convert_json_file_to_aif(params: Parameters):
    ...
    for data in claims_data:
        ...
        if source != prior_source:
            if af and '' != prior_source:
                af.close()
            aif_file = os.path.join(aif_dir, source + '.ttl')
            af = open(aif_file, "w")
            log.info('WRITING: ' + aif_file)
            af.write(
        ...
        claim_name = '<' + make_xml_safe('http://www.isi.edu/gaia/' + source + '/claim_id/' \
                                         + claim_id) + '>'
        ...
        if has_x_variable:
            x_variable_name = '<' + make_xml_safe('http://www.isi.edu/gaia/' + source + '/claim/'
                                                  + claim_id + '/x_variable/' + str(x_count)) + '>'
            x_variable_entity_name = '<' + make_xml_safe('http://www.isi.edu/gaia/' + source + '/claim/'
                                                         + claim_id + '/x_variable/entity/' + str(x_count)) + '>'
            if data['x_variable_type_qnode']['span'] is not None:
                ...
                x_variable_justification_name = '<' \
                                                + make_xml_safe(
                    'http://www.isi.edu/gaia/' + source + '/x_variable/justification' \
                    + str(x_count) + '/' + str(start_offset) + '/' + str(end_offset_inclusive)) + '>'
            x_count += 1
            af.write('\taida:xVariable ' + x_variable_name + ' ;\n')

        af.write('\taida:naturalLanguageDescription "' \
                 + reduce_whitespace(str(data['claim_sentence'])).replace('"', '') \
                 + '"^^xsd:string ;\n')

        has_claim_semantics = data['claim_semantics'] is not None and \
                              data['claim_semantics']['event'] is not None and \
                              data['claim_semantics']['event']['qnode_id'] is not None
        # alternative key check syntax: 'qnode_id' in data['claim_semantics']['event']
        claim_semantics = "None"
        if has_claim_semantics:
            claim_semantics = '<' \
                              + make_xml_safe('http://www.isi.edu/gaia/events/isi/qnode/claim-semantics-' \
        ...
        claimer_name = "None"
        if has_claimer:
            claimer_name = '<' + make_xml_safe('http://www.isi.edu/gaia/' + source \
                                               + '/claimer/' \
        ...
        claim_justification_name = '<' \
                                   + make_xml_safe('http://www.isi.edu/gaia/claims/justifications/' + source + '/' \
                                                   + str(start_offset) + '/' + str(end_offset_inclusive)) + '>'
        ...
        if has_x_variable:
            ...
            af.write('\taida:componentName "' + make_xml_safe(str(data['x_variable_type_qnode']['text'])) \
                     + '"^^xsd:string ;\n')
            af.write('\taida:componentType "' + str(data['x_variable_type_qnode']['qnode_id']) \
                     + '"^^xsd:string ;\n')
            if data['x_variable_identity_qnode'] is not None \
                and data['x_variable_identity_qnode']['qnode_id'] is not None:
                    af.write('\taida:componentIdentity "' + str(data['x_variable_identity_qnode']['qnode_id']) \
                             + '"^^xsd:string ;\n')
            af.write('\taida:componentKE ' + x_variable_entity_name + ' ;\n')
            af.write('\taida:privateData "[' + reduce_whitespace(str(data['x_variable_type_qnode'])) \
                     + ']"^^xsd:string .\n\n')

            af.write(x_variable_entity_name + ' a aida:Entity ;\n')
            af.write('\taida:text "' + make_xml_safe(str(data['x_variable_type_qnode']['text'])) \
                     + '"^^xsd:string ;\n')
            af.write('\taida:name "' + str(data['x_variable_type_qnode']['qnode_id']) \
                     + '"^^xsd:string ;\n')
            af.write('\taida:justifiedBy ' + x_variable_justification_name + ' ;\n')
            if data['x_variable']['entity'] is not None:
                af.write('\taida:privateData "[' + reduce_whitespace(str(data['x_variable']['entity'])) \
                         + ']"^^xsd:string ;\n')
        ...
        if has_claimer:
            ...
            af.write('\taida:componentName "' + str(data['claimer_type_qnode']['text']) \
                     + '"^^xsd:string ;\n')
            af.write('\taida:componentIdentity "' + str(data['claimer_type_qnode']['qnode_id']) \
                     + '"^^xsd:string ;\n')
            if data['claimer_identity_qnode'] is not None \
                    and data['claimer_identity_qnode']['qnode_id'] is not None:
                af.write('\taida:componentIdentity "' + str(data['claimer_identity_qnode']['qnode_id']) \
                         + '"^^xsd:string ;\n')
            af.write('\taida:privateData "[' + reduce_whitespace(str(data['claimer_type_qnode'])) \
                     + ']"^^xsd:string .\n\n')

        if has_claim_location:
            af.write('aida:' + claim_location + ' a aida:ClaimComponent ;\n')
            af.write('\taida:componentName "' + str(data['claim_location']) + '"^^xsd:string ;\n')
            af.write('\taida:componentIdentity "' + str(data['claim_location_qnode']) + '"^^xsd:string .\n\n')

        if has_claim_semantics:
            af.write(claim_semantics + ' a aida:Event ;\n')
            af.write('\taida:componentName "' \
                     + make_xml_safe(str(data['claim_semantics']['event']['text'])) \
                     + '"^^xsd:string ;\n')
            af.write('\taida:componentIdentity "' + str(data['claim_semantics']['event']['qnode_id']) \
                     + '"^^xsd:string ;\n')
            # TODO: args|source-patient/var|:identity:qnode_id or type

            af.write('\taida:privateData "[' \
